...
- Added `24_show_similarity_matrix.R`, a worked example of the similarity matrix
  with the manual answers marked.
- Deleted the R Markdown report in favour of this file.

## Corrections

- The ablation script computed the truth set after retrieval instead of before,
  dropping the positives retrieval had missed out of the recall denominator and
  inflating F1. A claimed +0.034 from fixing retrieval is withdrawn. Fixed, now
  agrees with the main cross validation to within 0.001.
- Coverage first reported as 43% / 79%, which was against reachable pairs.
  Against the full crosswalk it is 37.5% / 76.1%.
- Review burden first reported as 57%. That was share of pairs, not codes. It is
  14% of codes.
- Claimed nine codes had no ground truth and were silently skipped. Wrong, they
  are on the crosswalk's own exclusion list.

## Open

- Run label-only embeddings through the full pipeline.
- Block or sibling features for multi-target codes.
- Larger validation set. Currently 937 ICD-10-CA and 331 ICDA-8 pairs over 354
  three-digit ICD-9 codes. The CIHI crosswalk table would be closer to what the
  published LLM study evaluated on.
- Wider co-occurrence coverage. Second most important feature group, missing for
  many pairs.
