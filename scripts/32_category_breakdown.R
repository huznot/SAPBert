...
# one panel per system rather than two transparent histograms on top of each
...
dist <- dist %>% mutate(size_group = ifelse(n_codes == 1, "1 code", "2 or more codes"))
p <- ggplot(dist, aes(x = f1, fill = size_group)) +
  ...
  labs(title = "How F1 is spread across the CCS categories",
       subtitle = "held out. each bar counts categories scoring in that range, split by category size",
       x = "F1 within a category", y = "Number of categories", fill = NULL) +
  theme_minimal(base_size = 12) +
  theme(legend.position = "top", plot.title = element_text(face = "bold"),
        panel.grid.minor = element_blank(),
        strip.text.y = element_text(angle = 0, face = "bold"))
ggsave(file.path(OUT_DIR, "plot_ccs_f1_distribution.png"), p, width = 10, height = 6, dpi = 150)
